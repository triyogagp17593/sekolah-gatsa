...
<div class="right_col" role="main">
  ...
  <form action="<?= base_url('delete_guru_all')?>" method="post">
    <div class="row">
      <div class="col-md-12 col-sm-12 col-xs-12">
        <div class="x_panel">
          <div class="x_title">
            <h2>Data Guru / Staff<small><b>(tambah, ubah, hapus)</b></small></h2>
              ...
              <tbody>
                <?php 
                  ...
                  foreach ($data_pengguna->result() as $hasil) {
                    $cekajar = $this->main_model->get_all_sk_mengajarBy($hasil->id_login)->num_rows();
                ?>
                  <tr>
                    <td width="3%" align="center"><input type="checkbox" id="pilih[]" name="pilih[]" value="<?= $hasil->id_login;?>"></td>
                    <td width="3%" align="center"><?= $no++;?></td>
                    <td><?php if(empty($hasil->nipy)){echo"-";}else{echo $hasil->nipy;}?></td>
                    <td><?= $hasil->nama;?></td>
                    ...
                    <td width="15%" align="center">
                      <a href="<?= base_url('edit_guru/').$hasil->id_login;?>" id="ubah_admin" data-toggle="modal" data-target="#modal_edit_admin" class="btn btn-success btn-xs" title="ubah"><span class="fa fa-pencil"></span></a>
                      <a href="<?= base_url('delete_guru/').$hasil->id_login;?>" id="hapus" class="btn btn-danger btn-xs" title="hapus"><span class="fa fa-trash"></span></a>
                      <a href="<?= base_url('detail_guru/').$hasil->id_login;?>" id="detail_admin" data-toggle="modal" data-target="#modal_detail_admin" class="btn btn-info btn-xs" title="detail"><span class="fa fa-search"></span></a>
                      <?php if($cekajar < 1){ ?>
                        <a href="<?= base_url('ambil_mengajar_guru/').$hasil->id_login;?>" id="ambil_mengajar" data-toggle="modal" data-target="#modal_edit_ambil_mengajar" class="btn btn-warning btn-xs" title="ambil mengajar"><span class="fa fa-check"></span></a>
                      <?php } ?>
                    </td>
                  </tr>
                <?php } ?>
...
<div class="modal fade" id="modal_add_admin" tabindex="-1" role="dialog" aria-labelledby="largeModal" aria-hidden="true">
  <div class="modal-dialog" style="width: 1000px;">
    <div class="modal-content">
      ...
      <form action="<?= base_url('save_guru');?>" method="post" name="form" enctype="multipart/form-data" class="form-horizontal form-label-left">
        <div class="modal-body">
          <?php $this->load->view('dashboard/master_data/guru/add_guru');?>
        </div>
...
<div class="modal fade" id="modal_edit_ambil_mengajar" tabindex="-1" role="dialog" aria-labelledby="largeModal4" aria-hidden="true"></div>